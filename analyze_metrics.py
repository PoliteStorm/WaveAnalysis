...
def main():
    ...
    # Stage 1: Data Loading and Channel Selection
    with tqdm(total=3, desc="📊 Loading Data", ncols=80) as pbar:
    ...
    print(f"✅ Loaded {len(V)} samples from channel '{pick}'")

    # Stage 2: Spike Detection
    with tqdm(total=1, desc="🔍 Detecting Spikes", ncols=80) as pbar:
    ...
    with tqdm(total=3, desc="📈 Computing Statistics", ncols=80) as pbar:
    ...
    with tqdm(total=1, desc="🌀 √t Transform", ncols=80) as pbar:
        band_fracs = compute_tau_band_fractions(V, args.fs, tau_values, args.nu0)
        pbar.update(1)

    print(f"✅ Computed τ-band fractions for {len(tau_values)} time scales")
    ...
    with tqdm(total=2, desc="🧠 Advanced Metrics", ncols=80) as pbar:
        spike_train_metrics = compute_spike_train_metrics(spikes, len(V), args.fs)
        out['spike_train_metrics'] = spike_train_metrics
        pbar.update(1)

        multiscale_entropy = compute_multiscale_entropy(spikes, args.fs)
        out['multiscale_entropy'] = multiscale_entropy
        pbar.update(1)

    print(f"✅ Computed advanced metrics (Victor distance: {spike_train_metrics['victor_distance']:.2f})")
    print(f"✅ Multiscale entropy complexity: {multiscale_entropy['interpretation']}")

    # Stage 6: Stimulus-Response Validation (if applicable)
    ...
    print(f"📈 Found {len(spikes)} spikes with advanced metrics computed")
    print(f"🧠 Spike train complexity: {multiscale_entropy['interpretation']}")
    print(f"📁 Files generated: metrics.json, references.md")
    if args.plot:
        print(f"📊 Visualizations: heatmap, 3D surface, STFT vs √t, summary panel, histograms")
    if args.export_csv:
